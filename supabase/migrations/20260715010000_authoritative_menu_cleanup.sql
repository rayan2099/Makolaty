-- Canonical Makolaty menu synchronization.
-- Source of truth: the complete printed menu supplied on 2026-07-14.
-- This migration updates names, categories, prices, calories, size tiers, and ordering.
-- It also removes known obsolete records and exact duplicate rows created by earlier imports.

drop table if exists pg_temp.canonical_menu;

create temporary table canonical_menu (
  id text primary key,
...
insert into canonical_menu (
  id, "nameAr", "nameEn", category, price, calories, image, sizes, sort_order
)
select
  item ->> 'id',
  item ->> 'nameAr',
  ...
  item ->> 'category',
  (item ->> 'price')::numeric,
  case when item ? 'calories' then (item ->> 'calories')::integer else null end,
  coalesce(item ->> 'image', ''),
  item -> 'sizes',
  position - 1
...
-- These IDs represented duplicate or non-menu records in earlier seed versions.
...
-- Remove exact duplicate imports while retaining the canonical ID and its staff-edited image.
